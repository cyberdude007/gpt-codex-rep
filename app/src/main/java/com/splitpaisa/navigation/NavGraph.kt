...
import com.splitpaisa.feature.search.GlobalSearchScreen
import com.splitpaisa.feature.search.GlobalSearchViewModel
import com.splitpaisa.feature.parties.MemberSearchScreen
import com.splitpaisa.feature.parties.MemberSearchViewModel
...
fun PaisaNavGraph(
    ...
    NavHost(navController = navController, startDestination = Destinations.Home.route, modifier = modifier) {
        ...
        composable(Destinations.Search.route) {
            val context = LocalContext.current
            val vm: GlobalSearchViewModel = viewModel(factory = GlobalSearchViewModel.factory(context))
            GlobalSearchScreen(vm, onTransaction = {}, onCategory = {}, onParty = {})
        }
        ...
        composable(Destinations.MemberSearch.route) {
            val context = LocalContext.current
            val vm: MemberSearchViewModel = viewModel(factory = MemberSearchViewModel.factory(context, emptyList()))
            MemberSearchScreen(vm, onDone = { navController.popBackStack() })
        }
    }
}
...
sealed class Destinations(val route: String) {
    ...
    object MemberSearch : Destinations("memberSearch")
}
